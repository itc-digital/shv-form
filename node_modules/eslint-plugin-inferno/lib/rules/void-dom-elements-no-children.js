...
module.exports = {
  ...
  create: Components.detect((context, components, utils) => ({
    ...
    CallExpression: function(node) {
      ...
      if (!utils.isInfernoCreateElement(node)) {
        return;
      }
      ...
      if (args.length < 1) {
        // Inferno.createElement() should not crash linter
        return;
      }
      ...
      if (firstChild) {
        // e.g. Inferno.createElement('br', undefined, 'Foo')
        context.report({
          node: node,
          message: errorMessage(elementName)
        });
      ...
      if (hasChildrenPropOrDanger) {
        // e.g. Inferno.createElement('br', { children: 'Foo' })
        context.report({
          node: node,
          message: errorMessage(elementName)
        });
