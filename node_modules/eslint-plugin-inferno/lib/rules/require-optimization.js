...
module.exports = {
  meta: {
    docs: {
      description: 'Enforce Inferno components to have a shouldComponentUpdate method',
  ...
  create: Components.detect((context, components, utils) => {
    ...
    /**
     * Checks to see if our component is decorated by PureRenderMixin via infernoMixin
      ...
      if (node.decorators && node.decorators.length) {
        for (let i = 0, l = node.decorators.length; i < l; i++) {
          if (
            ...
            node.decorators[i].expression.callee.object.name === 'infernoMixin' &&
